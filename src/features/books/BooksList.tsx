...
const BooksList: React.FC = () => {
  ...
  if (booksState.totalItems < 1 && booksState.loading !== "pending")
    return null;
  if (booksState.loading === "pending" && booksState.searchOptions.page < 1)
    return (
      <div className="h-96">
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 w-full h-full">
        {booksState.fetchedBooks.map(({ volumeInfo }, index) =>
          volumeInfo ? (
            <BookCard key={index + 1} volumeInfo={volumeInfo} />
          ) : undefined
        )}
      </div>
      {isModalOpen ? <BookModal /> : undefined}
    </>
  );
};
